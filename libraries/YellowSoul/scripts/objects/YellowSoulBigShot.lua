...
local function approach(val, target, amount)
    if target < val then
...
function BigShot:update()
    if self.scale_x < 1 then
        self.scale_x = approach(self.scale_x, 1, 0.2*DTMULT)
    end
    if self.scale_y > 1 then
        self.scale_y = approach(self.scale_y, 1, 0.2*DTMULT)
    end
    if self.alpha < 1 then
        self.alpha = approach(self.alpha, 1, 0.2*DTMULT)
    end
    super.update(self)
end
