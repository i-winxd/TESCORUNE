        ...
        if Input.down("confirm") then -- charge a big shot
            ...
            if self.hold_timer >= 20 and not self.charge_sfx then -- start charging sfx
                ...
                Game.battle.timer:during(2/3, function()
                    timer = timer + DT
                    if self.charge_sfx then
                        self.charge_sfx:setVolume(clampMap(timer, 0,2/3, 0,0.3))
                    end
                end, function()
                    if self.charge_sfx then
            ...
            if self.hold_timer >= 20 and self.hold_timer < 40 then
                self.charge_sfx:setPitch(clampMap(self.hold_timer, 20,40, 0.1,1))
            end
        else
            self.hold_timer = 0
            if self.charge_sfx then
...
function YellowSoul:draw()
    ...
    -- Kristal.Console:log(string.format("r=%d,g=%d,b=%d,a=%d", r, g, b, a))
    local charge_timer = self.hold_timer - was35
    ...
    local circle_timer = math.min(self.hold_timer - was15, was35)
    if circle_timer > 0 then
        ...
        for i=1,4 do
            ...
            local scale = clampMap(circle_timer, 0,35, 4,2)
