...
function TescoBullet:onAdd()
    ...
    self.timer:script(function(wait)
        wait(0.5)
        self.timer:everyInstant(4.5, function()
        self.timer:script(function(wait)
            -- wait(0.01)
            ...
            if self.wave ~= nil then
                self.wave:spawnBullet("tesco_warning", final_x, final_y, angle_rad, pitch_table, 0.14)
            end
            wait(1)
...
function TescoBullet:onCollide(soul)
